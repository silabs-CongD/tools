...
# /opt
tools_folder = sys.argv[1]
tools_dir = os.path.join(os.environ.get('WORKSPACE'), tools_folder)

# Install Java17
def install_java():
	java_url = "https://artifactory.silabs.net/artifactory/studio-generic-production/java17/java17_linux_x86_64.zip"
	java_binary_path = os.path.join(tools_folder, "java17_linux_x86_64/jre/bin/java")
	if not os.path.isfile(java_binary_path):
		print("Downloading and unzipping java17 ...")
		dload.save_unzip(java_url, tools_folder, delete_after=True)
...
def install_sli_cli():
	...
	if not os.path.isfile(os.path.join(tools_folder, "slc_cli/slc")):
		print("Downloading and unzipping slc_cli ...")
		dload.save_unzip(slc_cli_url, tools_folder, delete_after=True)
		# Give execute permissions for slc-cli binary
		st = os.stat(os.path.join(tools_folder, "slc_cli", "slc"))
		os.chmod(os.path.join(tools_folder,"slc_cli", "slc"), st.st_mode | stat.S_IEXEC) 
	else:
...
def install_sim5():
	...
	sim5_path = os.path.join(tools_folder, "SimplicityStudio_v5")
	if not os.path.isfile(os.path.join(sim5_path, "studio")):
		print("Downloading and unzipping Sim5 ...")
		dload.save(sim5_url, os.path.join(tools_folder, "ss.tar.gz"))
		tar = tarfile.open(os.path.join(tools_folder,"ss.tar.gz"), "r:gz")
		tar.extractall(path=tools_folder)
		tar.close()
		os.remove(tools_dir + "/ss.tar.gz")
	else:
...
def install_gcc_12():
	...
	arm_gcc_dir = os.path.join(tools_folder, "arm-gnu-toolchain-12.2.rel1-x86_64-arm-none-eabi")
	...
	if not os.path.isfile(os.path.join(arm_toolchain_path, "arm-none-eabi-gcc")):
		...
		dload.save(arm_toolchain_url, os.path.join(tools_folder, "gcc.tar.xz"))
		tar = tarfile.open(os.path.join(tools_folder, "gcc.tar.xz"), "r:xz")  
		tar.extractall(path=tools_folder)
		tar.close()
		os.remove(tools_dir + "/gcc.tar.xz")
	else:
